...
import { readableStreamToString, SeelError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SeelError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SeelError(e, e.response?.data)
                }
                if (innerError instanceof SeelError) {
                    // Got "SeelError" from the above try block
                    throw innerError;
                }
